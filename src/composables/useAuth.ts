...
export function useAuth() {
  ...
  const hasActiveSubscription = computed(() => user.value?.subscriptionStatus === 'active')
  ...
  const signInWithGoogle = async (isSignup = false) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/auth/google?isSignup=${isSignup}`)
      const data = await response.json()
  ...
  const handleAuthSuccess = async (authData: AuthResponse, wasSignup: boolean) => {
    try {
      ...
      alert('Authentication completed but failed to process. Please try again.')
    } finally {
      isLoading.value = false
    }
  }
